...
def random_forest():
    ...
    normalizer = TextNormalizer(emoji='text')  # emoji
    labels = ["antisemistic", "not_antisemistic", "irrelevant"]  # irrelevant

    classifier = SpacyClassifier(nlp, normalizer, labels)

    data = classifier.load_data(set_to_min=True)

    data = classifier.preprocess_data(data)  # <- the tokenizer is here
